...
class IntegrationTests {
     ...
     */
    int countIntersections(Intersectable geometry) {
        List<Point> l1;
        int size = 0;
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 3; ++j) {
                l1 = geometry.findIntersections(camera.constructRay(3, 3, j, i));
                if (l1 != null) {
                    size += l1.size();
                }
            }
